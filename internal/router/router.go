...
func Setup(db *gorm.DB, assistantCfg config.AssistantConfig, ck *ckclient.Client, table string) *gin.Engine {
	...
	r.Use(cors.New(cors.Config{
		AllowOrigins: []string{"http://localhost:5173", "http://127.0.0.1:5173", "http://test.gpuhealth-ui.jd.local"},
	...
	healthH := handler.NewHealthHandler(healthRepo, metricRepo, faultEventRepo, topoRepo)
	...
	api := r.Group("/api/v1")
	{
		...
		api.GET("/health/gpus/:uuid/score-trend", metricSeriesH.ScoreTrend)
