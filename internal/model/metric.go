...
type MetricDefinition struct {
	ID          uint64 `gorm:"primaryKey;autoIncrement" json:"id"`
	SeqNo       *int   `gorm:"type:int" json:"seq_no"`                                    //原始excel表中编号
	MetricName  string `gorm:"type:varchar(191);uniqueIndex;not null" json:"metric_name"` // 指标名称(DCGM字段或NPU字段)
	OfficialNum string `gorm:"column:official_no;type:varchar(191)" json:"official_no"`   // 官方编号或说明
	CardType    string `gorm:"type:varchar(64);index" json:"card_type"`                   //卡的类型（GPU/NPU）
	Conception  string `gorm:"column:concept;type:text" json:"concept"`                   // 概念说明
	Unit        string `gorm:"type:varchar(32)" json:"unit"`                              // 指标单位
	Dimension   string `gorm:"type:varchar(64);index" json:"dimension"`                   // 指标维度

	//归属主体与健康度评分用途
	OwnerSubject  int `gorm:"column:owner_subject_code;type:tinyint;index"  json:"owner_subject"`  //归属主体：A单卡自身/B链路/C共享/D环境/E跨节点
	HealthPurpose int `gorm:"column:health_purpose_code;type:tinyint;index" json:"health_purpose"` //健康度评分用途：核心/归因/配置合规/性能容量
	ValueType     int `gorm:"column:value_type_code;type:tinyint;index"     json:"value_type"`     //指标的数值类型：Gauge连续数值/Gauge_Rate比率/Counter累计计数...

	WorkRange    string   `gorm:"type:varchar(128)" json:"work_range"`               // 工作范围，如 0~85
	UpperBond    *float64 `gorm:"column:upper_bound;type:double" json:"upper_bound"` //连续数值上界
	LowerBound   *float64 `gorm:"type:double"                    json:"lower_bound"` //连续数值下界
	WarnupBound  *float64 `gorm:"column:alert_upper;type:double" json:"alert_upper"` //警告/故障上界
	WarnlowBound *float64 `gorm:"column:alert_lower;type:double" json:"alert_lower"` //警告/故障下界

	NormalRate     *float64 `gorm:"type:double"                    json:"normal_rate"`  // 正常速率上限
	WarnRate       *float64 `gorm:"column:alert_rate;type:double"  json:"alert_rate"`   // 警告速率上限
	NormalRateUnit string   `gorm:"column:rate_unit;type:varchar(32)" json:"rate_unit"` // 次/秒 次/分钟 次/小时 次/天 μs/s

	BoolNormal   string `gorm:"type:text" json:"bool_normal"`   //正常的bool值
	BoolAbnormal string `gorm:"type:text" json:"bool_abnormal"` //异常的bool值
	EnumResult   string `gorm:"type:text" json:"enum_result"`   //修改枚举类型
	EnumScore    string `gorm:"type:json" json:"enum_score"`    //枚举/位掩码的结构化评分规则

	IsVeto          int    `gorm:"type:tinyint;index" json:"is_veto"`    //是否“一票否决”：1是、0否
	DerateThreshold string `gorm:"type:text" json:"derate_threshold"`    //降频/关机阈值说明
	SourceRef       string `gorm:"type:text" json:"source_ref"`          //来源依据
...
func (MetricDefinition) TableName() string { return "accel_metric_scoring" }
