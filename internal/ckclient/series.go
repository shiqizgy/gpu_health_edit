...
func (c *Client) QuerySeries(ctx context.Context, table, gpuSN, mib string,
	...
	q := fmt.Sprintf(`
SELECT toStartOfInterval(timestamp, INTERVAL %d SECOND) AS ts, ifNull(%s(value), 0) AS v
FROM %s
WHERE gpu_sn = ? AND mib = ? AND value IS NOT NULL
    AND toDate(dt) BETWEEN toDate(?) AND toDate(?) AND timestamp BETWEEN ? AND ?
GROUP BY ts ORDER BY ts`, bucketSec, agg, table)

	rows, err := c.db.QueryContext(ctx, q, gpuSN, mib, from, to, from, to)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (c *Client) QueryEvents(ctx context.Context, table, gpuSN, mib string,
	from, to time.Time) ([]SeriesPoint, error) {

	q := fmt.Sprintf(`
SELECT timestamp AS ts, value AS v
FROM %s
WHERE gpu_sn = ? AND mib = ? AND value > 0
    AND toDate(dt) BETWEEN toDate(?) AND toDate(?) AND timestamp BETWEEN ? AND ?
ORDER BY ts`, table)
