...
func (c *Client) QuerySeries(ctx context.Context, table, sn, tags, mib string,
	...
	// 若 CK 的 value 列是 String 类型，把 %s(value) 换成 %s(toFloat64OrZero(value))
	q := fmt.Sprintf(`
SELECT toStartOfInterval(timestamp, INTERVAL %d SECOND) AS ts, %s(value) AS v
FROM %s
WHERE sn = ? AND tags = ? AND mib = ?
  AND dt BETWEEN ? AND ? AND timestamp BETWEEN ? AND ?
GROUP BY ts ORDER BY ts`, bucketSec, agg, table)

	rows, err := c.conn.Query(ctx, q, sn, tags, mib, from, to, from, to)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
