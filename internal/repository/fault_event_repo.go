...
// PurgeResolvedBefore 分批删除 resolved_at 早于 cutoff 的历史故障（保护 open 事件）。
// 返回累计删除行数。
func (r *FaultEventRepo) PurgeResolvedBefore(cutoff time.Time, batch int) (int64, error) {
	if batch <= 0 {
		batch = 1000
	}
	var total int64
	for {
		res := r.db.Where("status = ? AND resolved_at IS NOT NULL AND resolved_at < ?", "resolved", cutoff).
			Limit(batch).Delete(&model.FaultEvent{})
		if res.Error != nil {
